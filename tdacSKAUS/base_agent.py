        ...
        for episode in range(self.total_episode):
            ...
            for t in range(10000):
                if self.total_time_step < self.warmup_steps:
                    # austria from scratch ===========================
                    action = [0.3, 0.008] + np.random.normal(loc=0, scale=0.05, size=(2))
                    if t > 1 :
                        pos = info['progress']
                        if pos > 0.105 and pos < 0.132:
                            action[1] += 1.0
                        if pos > 0.25 and pos < 0.28:
                            action[1] += -0.17
                        if pos >= 0.36:
                            teacher_pos = pos
                            action = self.masters_agent_actions(stacked_frames, stacked_velocity, sigma=0)
                else:
                    sigma = max(0.1*(1 - episode/self.total_episode), 0.01)
                    if t > 1 :
                        if episode < 1000:
                            if self.method == "episode":
                                if episode % 2 == 0:
                                ...
                                action0 = self.masters_agent_actions(stacked_frames, stacked_velocity, sigma=sigma)
                                action1 = self.decide_agent_actions(stacked_frames, stacked_action, sigma=sigma)
                                action = (action0 + action1) / 2
                        else:
                            stu_pos = info['progress']
                            if stu_pos > max_pos:
                                max_pos = stu_pos
                ...
                if (teacher_pos >= max_pos + 0.2) and (self.method == "episode"):
                    terminates = 1
