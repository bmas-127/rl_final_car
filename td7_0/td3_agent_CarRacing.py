...
import numpy as np
from td7_0.base_agent import TD3BaseAgent
from td7_0.models.CarRacing_model import ActorNetSimple, CriticNetSimple
import random
from td7_0.base_agent import OUNoiseGenerator, GaussianNoise
from racecar_gym.env import RaceEnv
import gymnasium as gym
from td7_0.environment_wrapper.Env import Env
...
class CarRacingTD3Agent(TD3BaseAgent):
    def __init__(self, config):
        ...
        # --------------------------
        # target network
        self.target_actor_net = ActorNetSimple(self.observation_space, self.action_space, self.feature_space, self.num_frames)
        self.target_critic_net1 = CriticNetSimple(self.observation_space, self.action_space, self.feature_space, self.num_frames)
        self.target_critic_net2 = CriticNetSimple(self.observation_space, self.action_space, self.feature_space, self.num_frames)
    ...
    def decide_agent_actions(self, state, velocity, sigma=0.0, brake_rate=0.015): 
        ### TODO ###
        ...
        with torch.no_grad():   
            ...
            action = self.actor_net(state, velocity).cpu().data.numpy() + sigma * self.noise.generate()
    ...
    def update_behavior_network(self):
        ...
        state, velocity, action, reward, next_state, next_velocity, done = self.replay_buffer.sample(self.batch_size, self.device)
        ### TODO ###
        ...
        ## Update Critic ##
        # critic loss
        q_value1 = self.critic_net1(state, velocity, action)
        q_value2 = self.critic_net2(state, velocity, action)

        
        with torch.no_grad():
            # select action a_next from target actor network and add noise for smoothing

            a_next = self.target_actor_net(next_state, next_velocity).cpu().data.numpy() 
            ...
            q_next1 = self.target_critic_net1(next_state, next_velocity, a_next)
            q_next2 = self.target_critic_net2(next_state, next_velocity, a_next)
        ...
        if self.total_time_step % self.update_freq == 0:
            ## Update Actor ##
            # actor loss
            action = self.actor_net(state, velocity)
            actor_loss = -1 * self.critic_net1(state, velocity, action).mean()
            # optimize actor
